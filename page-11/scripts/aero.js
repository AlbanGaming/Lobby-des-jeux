function getAeroContent() {
    return `
        <h2 style="color: var(--primary-color); margin-bottom: 20px;">✈️ Recherche & Développement Aérodynamique</h2>
        <div class="main-grid">
            <div class="section">
                <h3>🎯 Actions</h3>
                <div class="clicker-zone">
                    ...
                    <div class="action-selector" style="margin-top: 20px;">
                        <select id="aeroActionSelect" onchange="changeAction(this.value)" style="background: rgba(0, 0, 0, 0.2); border: 1px solid var(--primary-color); color: var(--text-color);">
                            <option value="aeroWork" selected>✈️ Travail Aéro (selon dev en cours)</option>
            ...
            <div class="section">
                <h3>📊 Classement Performance Aéro</h3>
                <table>
                    <thead>
                        <tr>
                            <th>Pos</th>
                    ...
                    <tbody>
                        ${generateAeroRankings()}
                    </tbody>
                </table>
            </div>
        </div>
        <div class="section" style="margin-top: 20px;">
            <h3>🔧 Arbre de Développement Aérodynamique</h3>
            <div class="dev-tree">
                ${Object.entries(aeroParts).map(([key, part]) => {
                    const dev = gameState.aeroDev[key] || { phase: 'locked', progress: 0 };
                    ...
                    return `
                        <div class="dev-item ${isLocked ? 'locked' : ''} ${isCompleted ? 'completed' : ''}" style="border: 2px solid ${isCompleted ? '#4ecca3' : isLocked ? '#555' : 'var(--primary-color)'};">
                            ...
                            ${isCompleted ?
                                ...
                                isLocked ?
                                    `<button class="small-btn" onclick="startDevelopment('aero', '${key}')" style="background: var(--primary-color); border: 1px solid #fff;">Démarrer</button>` :
                                    `
...
function generateAeroRankings() {
    const rankings = [...gameState.constructorStandings];
    rankings.sort((a, b) => b.aeroPerf - a.aeroPerf);
    return rankings.slice(0, 11).map((team, index) => `
        <tr style="${team.team === gameState.teamName ? 'background: rgba(233, 69, 96, 0.1);' : ''}">
            ...
            <td>${Math.round(team.aeroPerf)} pts</td>
        </tr>
    `).join('');
}
