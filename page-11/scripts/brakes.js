            ...
            <div class="section">
                ...
                <div class="clicker-zone">
                    ...
                    <div class="action-selector" style="margin-top: 20px;">
                        <select id="brakesActionSelect" onchange="changeAction(this.value)" style="background: rgba(0, 0, 0, 0.2); border: 1px solid var(--primary-color); color: var(--text-color);">
                            <option value="brakesWork" selected>🛑 Travail Freins (selon dev en cours)</option>
                    ...
                    return `
                        <div class="dev-item ${isLocked ? 'locked' : ''} ${isCompleted ? 'completed' : ''}" style="border: 2px solid ${isCompleted ? '#4ecca3' : isLocked ? '#555' : 'var(--primary-color)'};">
                            ...
                            ${isCompleted ?
                                ...
                                isLocked ?
                                    `<button class="small-btn" onclick="startDevelopment('brakes', '${key}')" style="background: var(--primary-color); border: 1px solid #fff;">Démarrer</button>` :
                                    `
...
function generateBrakesRankings() {
    ...
    rankings.sort((a, b) => b.brakesPerf - a.brakesPerf);
    return rankings.slice(0, 11).map((team, index) => `
        <tr style="${team.team === gameState.teamName ? 'background: rgba(233, 69, 96, 0.1);' : ''}">
            ...
            <td>${Math.round(team.brakesPerf)} pts</td>
        </tr>
    `).join('');
}
